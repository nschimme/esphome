...
async def to_code(config):
    output_var = cg.new_Pvariable(config[light.CONF_OUTPUT_ID])
    await cg.register_component(output_var, config)
    await register_client_entity(output_var, config)
    if CONF_COLOR_TEMPERATURE in config:
        cg.add(output_var.set_color_temperature(config[CONF_COLOR_TEMPERATURE]))
    await light.register_light(output_var, config)
