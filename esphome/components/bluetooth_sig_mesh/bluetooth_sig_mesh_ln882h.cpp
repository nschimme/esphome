...
void LN882HBluetoothSIGMesh::init_ln882h_mesh_() {
  ESP_LOGI(TAG, "Initializing LN882H BLE Mesh driver...");
  ...
  if (ln882h_ble_tracker::global_ln882h_ble_tracker != nullptr) {
    ESP_LOGD(TAG, "Configured LN882H BLE controller activity for SIG Mesh");
...
void LN882HBluetoothSIGMesh::send_mesh_pdu(uint16_t dst, uint16_t app_idx, uint16_t opcode, const uint8_t *payload,
  ...
  ESP_LOGI(TAG, "Framed LN882H BLE Mesh encrypted advertisement frame (DST: 0x%04X, Framed Len: %zu)", dst,
           framed_pdu.size());
}
